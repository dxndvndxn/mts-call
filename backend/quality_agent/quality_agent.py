...
rules = ("1. Ответы должны быть только по тематике деятельности компании"
         ...
         "5. Нельзя придумывать условия и продукты, которых нет у компании"
         "6. Если запрос клиента не ясен, нужно уточнить у клиента, что он имеет в виду, предложив ему наиболее подходящие варианты из базы знаний")
...
def reasoning_step_two(chat, intent, emotion, action, rs_1):
...
def quality_agent(chat, intent, emotion, action):
    rs_1 = reasoning_step_one(chat, intent, emotion, action, rules)
    rs_2 = reasoning_step_two(chat, intent, emotion, action, rs_1)
